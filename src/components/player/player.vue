          ...
          <scroll class="middle-r" ref="lyricList" :data='currentLyric && currentLyric.lines'>
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p ref="lyricLine" :class="{'current' : currentLineNum === index}" class="text" v-for="(item, index) in currentLyric.lines" :key="index">{{item.txt}}</p>
              </div>
            </div>
          </scroll>
        </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['fullScreen', 'playList', 'currentSong', 'playing', 'currentIndex', 'mode', 'squenceList'])
  },
  methods: {
    middleTouchStart(e) {
      console.log('111')
    ...
    format(time) {
      ...
      const secend = (time % 60).toString().padStart(2, 0)
      return `${minute}:${secend}`
    },
    getLyric() {
      this.currentSong.getLyric().then((res) => {
        this.currentLyric = new Lyric(res, this.handleLycir)
        if (this.playing) {
